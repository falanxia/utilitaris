package Box2D.Collision{
	...
	public class b2Segment {
		...
		public function ExtendBackward(aabb:b2AABB):void {
			...
			var lambda:Number = Math.min(dX > 0 ? (aabb.upperBound.x - p2.x) / dX : dX < 0 ? (aabb.lowerBound.x - p2.x) / dX : Number.POSITIVE_INFINITY, dY > 0 ? (aabb.upperBound.y - p2.y) / dY : dY < 0 ? (aabb.lowerBound.y - p2.y) / dY : Number.POSITIVE_INFINITY);
